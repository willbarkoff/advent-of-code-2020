...
func main() {
	...
	for scanner.Scan() {
		passwordSpec := scanner.Text()
		passwordSpecArr := strings.Split(passwordSpec, " ")

		bounds := strings.Split(passwordSpecArr[0], "-")
		...
		char := passwordSpecArr[1][0]
		pw := passwordSpecArr[2]

		if (pw[first-1] == char && pw[second-1] != char) || (pw[first-1] != char && pw[second-1] == char) {
			validPasswords++
		}
