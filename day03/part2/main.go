...
func main() {
	...
	for _, v := range slopesToCheck {
		multiplied *= checkSlope(v[0], v[1])
	}
...
func checkSlope(right, down int) int {
	...
	for scanner.Scan() {
		...
		forestMap = append(forestMap, currentRow)
		maxRowLength = len(line)
	}

	treeCount := 0

	for i := 0; i*down < len(forestMap); i++ {
		if forestMap[i*down][(i*right)%maxRowLength] {
			treeCount++
		}
	}
